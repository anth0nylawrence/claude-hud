...
export function render(ctx: RenderContext): void {
  ...
  // Grid layout: 4x4 agent grid with main session (box drawing has its own borders)
  if (layout === 'grid') {
    const gridLines = renderAgentsGrid(ctx);
    if (gridLines.length > 0) {
      lines.push(...gridLines);
    }

    // Still show tools line in grid layout
    ...
    // Default and separators layouts: single-line activity display
